...
class LASERBackend:
    ...
    def _detect_lite(
        ...
        # Debug output - only when speaking state changes (not constant spam)
        if self.config.debug and face.track_id == 0:  # Only for first face
            if not hasattr(self, '_last_speaking_state'):
                self._last_speaking_state = {}
            
            last_state = self._last_speaking_state.get(face.track_id, None)
            
            # Only print when state changes
            if last_state != is_speaking:
                self._last_speaking_state[face.track_id] = is_speaking
                if is_speaking:
                    print(f"[ASD] Speaking detected (MAR={current_mar:.3f}, prob={probability:.2f})")
                # Don't print when stopping - too noisy
